...
import { translations } from "@synoem/transactional/newsletter-confirmation";
import { type CollectionAfterOperationHook, APIError } from "payload";
import { render, pretty } from "@react-email/render";
import React from "react";
import { NewsletterConfirmation } from "@synoem/transactional/newsletter-confirmation";
...
> = async ({ operation, req, result }) => {
  if (operation === "create") {
    ...
    if (!isValidLocale(locale)) {
      throw new APIError("Invalid locale");
    }
